...
@Component({
  selector: 'app-form-approve-hoi-dong-nghiem-thu',
  templateUrl: './form-approve-hoi-dong-nghiem-thu.component.html',
  styleUrls: ['./form-approve-hoi-dong-nghiem-thu.component.scss']
})
export class FormApproveHoiDongNghiemThuComponent implements OnInit {
  @Input() modalDataDuyet: ModalData<HoiDongNghiemThu>;
  ...
  ngOnInit() {
    this.createForm();
    this.mapHoiDong();
    if (this.modalDataDuyet.action === SystemConstant.ACTION.EDIT) {
    ...
    this.listChucVuHoiDongTemp = this.listChucVuHoiDong.concat(this.listChucVuHoiDongDeXuat);
  ...
  createForm() {
    ...
    this.listThanhVien = [...this.modalDataDuyet.data.thanhVienHoiDongs.map(x => ({
      ...
      trangThaiDuyetThanhVien: x.nhiemVuHoiDong === SystemConstant.CHUC_VU_HOI_DONG_KHOI_TAO.CHU_TICH
        || x.nhiemVuHoiDong === SystemConstant.CHUC_VU_HOI_DONG_KHOI_TAO.THU_KY ? true : null,
      isEdit: false,
    }))];
  ...
  checkNhanVienExist(email: string) {
    return this.modalDataDuyet.data.thanhVienHoiDongs.some(x => x.email === email);
  }
  ...
  mappingTrangThaiDuyetThanhVien() {
    this.listThanhVien.map(x => {
      if (x.trangThaiDuyetThanhVien === null) {
        x.trangThaiDuyetThanhVien = false;
      }
    });
  }

  onSubmit() {
    this.mappingTrangThaiDuyetThanhVien();
    this.hoiDongNghiemThuSvc.duyetDeXuatHoiDongNghiemThu(this.modalDataDuyet.data.id,
      this.listThanhVien)
      .subscribe(() => {
