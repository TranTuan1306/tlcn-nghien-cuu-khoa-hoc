...
import { DeTai, ThongTinKetQua } from 'src/app/core/models/management/de-tai/de-tai.model';
...
@Component({
  selector: 'app-list-research-results',
  templateUrl: './list-research-results.component.html',
  styleUrls: ['./list-research-results.component.scss', '../../../../assets/theme/css/main.css', '../../../../assets/theme/css/main.css']
})
export class ListResearchResultsComponent implements OnInit {
  ...
  routerLinkToFormResearchResult(id: string) {
    this.spinner.show();
    setTimeout(() => {
      this.router.navigate(['/work/research-results', id]);
    }, 200);
  }
  ...
  clickToDownloadBieuMau1011(id: string) {
    this.spinner.show();
    this.bieuMauSvc.xuatBieuMauKiemTraThongTinKetQuaNghienCuu(id)
      .subscribe(res => {
        this.convertFileFromBlob(res.body, `BM10T-va-BM11T-bieu-mau-thong-tin-ket-qua-nghien-cuu${id}.docx`);
        this.spinner.hide();
      }, () => this.spinner.hide());
  }
  ...
  checkThongTinKetQua(thongTinKetQua: ThongTinKetQua) {
    if (!thongTinKetQua) {
      return false;
    } else {
      return true;
